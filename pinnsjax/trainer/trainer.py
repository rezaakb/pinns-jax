...
class Trainer:
    ...
    def callback_pbar(self, loss_name, loss, extra_variables=None, extra_variables_values=None):
    ...
    def set_callback_metrics(self, loss_name, loss, extra_variables=None, extra_variables_values=None):
        self.callback_metrics[loss_name] = loss
    ...
    def initalize_tqdm(self, max_epochs):
        return tqdm(
                total= max_epochs,
                bar_format="{n_fmt}/{total_fmt} {percentage:3.0f}%|{bar}|[{elapsed}<{remaining}, "
                "{rate_fmt}{postfix}, "
                "{desc}]",
                )
        ...
        for epoch in range(self.current_epoch, self.max_epochs):
            ...
            if datamodule.batch_size is not None:
                ...
                loss, extra_variables, model.trainable_variables, model.opt_state = model.train_step(model.trainable_variables,
                                                                                                     model.opt_state, train_data)
                
            else:
                loss, extra_variables, model.trainable_variables, model.opt_state = model.train_step(model.trainable_variables,
                                                                                                     model.opt_state, train_dataloader)

            elapsed_time = time.time() - start_time
            self.time_list.append(elapsed_time)

            self.set_callback_metrics('train/loss', loss, extra_variables, model.trainable_variables)
            
            if self.enable_progress_bar:
                self.pbar.update(1)
                description = self.callback_pbar('train/loss', loss, extra_variables, model.trainable_variables)
